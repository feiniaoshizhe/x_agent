...
async def load_mcp_servers_from_db() -> None:
    ...
    try:
        async with db_manager.get_async_session_context() as session:
            result = await session.execute(select(MCPServer).filter(MCPServer.enabled == True))
...
async def toggle_server_enabled(db: AsyncSession, name: str, updated_by: int = None) -> tuple[bool, MCPServer]:
    """Toggle server enabled status."""
    server = await get_mcp_server(db, name)
    if not server:
        raise ValueError(f"Server '{name}' does not exist")

    server.enabled = False if server.enabled else True
    if updated_by is not None:
        server.updated_by = updated_by
    await db.commit()
